       ...
       export const postMsg = async(req,res)=>{
        try{
            ...
            if(post._id){
                ...
                res.status(404).send({status:"fail",dada:"Failed to create contact"})
            }
        }catch(err){
...
export const deleteContact = async(req,res)=>{
    ...
    try {
        ...
        res.status(200).send({ status:"success", message: `There are ${contactCount} contacts in the collection.` });
    } catch (err) {
